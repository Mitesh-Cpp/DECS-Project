...
{
    int sockfd, newsockfd, portno, thread_num, thread_num2=1; 
    socklen_t clilen;
    ...
    initialize();//setting up requestID set
    ...
    {
        if (pthread_create(&worker_thread[i], NULL, &receive_file, NULL) != 0) // P
        {
    ...
    {
        if (pthread_create(&request_handler_thread[i], NULL, &handle_request, NULL) != 0) // P
        {
